...
| Performance smoke check           | `pnpm nfr:performance`                           | Frontend and backend services running at the configured base URLs; route set and request count defined by the script.        | Manual/release check. Do not add to PR CI unless services and data become stable there.  | Current evidence summary or a small dated evidence summary.                                            | Fails if p95 latency exceeds `2000ms` or error rate exceeds `0.01` for the configured route set.                                    | Record actual results, fix the bottleneck or environment issue, and rerun the same route set.                                               |
| Performance configuration dry-run | `pnpm nfr:performance:dry-run`                   | Local repository checkout; no services required.                                                                             | Stable local configuration check.                                                        | Current evidence summary.                                                                              | Fails if the route set or thresholds are invalid.                                                                                   | Fix the script configuration before running the service-backed smoke check.                                                                 |
