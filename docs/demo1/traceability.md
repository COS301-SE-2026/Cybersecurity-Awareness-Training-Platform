...
| UC-02: View Training Document         | UC-02 user story in `SRS.md`     | UC-02 core use case         | FR-UC02-01 to FR-UC02-10; TRK-UC02-01 to TRK-UC02-04                         | API-UC02-01 to API-UC02-03; API-TRK-02               | `User`, `LearnerProfile`, `CampaignAssignment`, `CampaignItem`, `TrainingDocument`, `InteractionEvent`                         | `DESIGN.md` Training Module List and Training Material Page            | QA-UC02-01 to QA-UC02-05              | Frontend training list/document screens; backend training interaction APIs  | Connor  | Integrated draft   |
| UC-03: Complete Quiz Flow             | UC-03 user story in `SRS.md`     | UC-03 core use case         | FR-UC03-01 to FR-UC03-10; TRK-UC03-01 to TRK-UC03-06                         | API-UC03-01 to API-UC03-04; API-TRK-03 to API-TRK-05 | `User`, `LearnerProfile`, `CampaignItem`, `Quiz`, `QuizQuestion`, `AnswerOption`, `QuizAttempt`, `AttemptAnswer`, `QuizResult` | `DESIGN.md` Quiz Page, Submission State, and Results Page              | QA-UC03-01 to QA-UC03-05              | Frontend quiz/results screens; backend quiz attempt/submission/result APIs  | Zoë     | Integrated draft   |
| Tracking/Progress/Reporting Support   | Supporting tracking requirements | UC-01, UC-02, UC-03 support | TRK-DEMO1-01 to TRK-DEMO1-08; RPT-DEMO1-01 to RPT-DEMO1-06                   | API-TRK-01 to API-TRK-06                             | `InteractionEvent`, `EmailClassificationResponse`, `QuizAttempt`, `QuizResult`, `ReportSummary`, `RiskIndicator`               | Domain model tracking/reporting support; no new design screen required | QA tracking/progress placeholders     | Future reporting/risk placeholders only; no dashboard implementation        | Adriano | Integrated draft   |
...
| --------------- | ---------------------------------- | ------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------ |
| TRACE-TRK-01    | Simulated email opened/viewed      | UC-01               | TRK-UC01-01, TRK-UC01-02, TRK-UC01-03, FR-UC01-04 | API-TRK-01 / `POST /learner/simulated-emails/:emailId/opened`                        | `InteractionEvent`, `SimulatedEmail`, `LearnerProfile`                           | Records lightweight simulated inbox interaction events.                                | Draft  |
...
| TRACE-TRK-03    | Training document viewed/completed | UC-02               | TRK-UC02-01, TRK-UC02-02, TRK-UC02-03, FR-UC02-04 | API-TRK-02 / `POST /learner/campaign-items/:campaignItemId/training-document/viewed` | `TrainingDocument`, `CampaignItem`, `InteractionEvent`, `LearnerProfile`         | Tracks high-level training engagement through lightweight interaction events.          | Draft  |
...
| TRACE-TRK-05    | Quiz attempt creation              | UC-03               | TRK-UC03-01, FR-UC03-03                           | API-TRK-03 / `POST /learner/campaign-items/:campaignItemId/quiz-attempts`            | `QuizAttempt`, `Quiz`, `LearnerProfile`                                          | Starting a quiz creates an attempt.                                                    | Draft  |
...
| TRACE-TRK-07    | Quiz result and feedback           | UC-03               | TRK-UC03-04, TRK-UC03-05, FR-UC03-07, FR-UC03-08  | API-TRK-05 / `GET /quiz-attempts/:attemptId/result`                                  | `QuizResult`, `AnswerOption`, `AttemptAnswer`                                    | Submitted attempts can produce result summaries and answer-level educational feedback. | Draft  |
...
| User types               | Admin, Trainee                  | Auth/user context                                                       | `User.userType`                                          | Aligned | `Trainee` covers organisation-linked and general learning-flow accounts; actor wording remains `trainee`. |
| Campaign wrapper         | Campaign                        | `POST /campaigns`                                                       | `Campaign`                                               | Aligned | Supporting context only for Demo 1.                                                                       |
| Campaign assignment      | Campaign assignment context     | `POST /campaigns/:campaignId/assign`                                    | `CampaignAssignment`                                     | Aligned | Assignment links a `Campaign` to a `LearnerProfile`.                                                      |
| Simulated inbox          | Simulated inbox                 | `GET /learner/campaign-items/:campaignItemId/simulated-inbox`           | `SimulatedInbox`                                         | Aligned | Represents controlled campaign-provided inbox content only.                                               |
| Simulated email          | Simulated email                 | `GET /learner/simulated-emails/:emailId`                                | `SimulatedEmail`                                         | Aligned | Does not represent real mailbox email delivery.                                                           |
| Simulated email tracking | Email opened/viewed interaction | `POST /learner/simulated-emails/:emailId/opened`                        | `InteractionEvent`                                       | Aligned | Used for lightweight UC-01 interaction tracking.                                                          |
| Email classification     | Email classification response   | `POST /learner/simulated-emails/:emailId/classification`                | `EmailClassificationResponse`, `EmailRedFlag`            | Aligned | Records classification separately from quiz attempts.                                                     |
| Training list/content    | Training document               | `GET /learner/campaign-items/:campaignItemId/training-document`         | `CampaignAssignment`, `CampaignItem`, `TrainingDocument` | Aligned | Training documents are reusable content placed into campaigns through campaign items/components.          |
| Training interaction     | Training interaction            | `POST /learner/campaign-items/:campaignItemId/training-document/viewed` | `InteractionEvent`                                       | Aligned | Records high-level training interaction only.                                                             |
| Quiz content             | Quiz                            | `GET /learner/campaign-items/:campaignItemId/quiz`                      | `Quiz`, `QuizQuestion`                                   | Aligned | Supports UC-03 quiz display.                                                                              |
| Quiz attempt             | Quiz attempt                    | `POST /learner/campaign-items/:campaignItemId/quiz-attempts`            | `QuizAttempt`                                            | Aligned | Created when quiz starts.                                                                                 |
...
| Quiz results/feedback    | Quiz results and feedback       | `GET /quiz-attempts/:attemptId/result`                                  | `QuizResult`, `AnswerOption`, `AttemptAnswer`            | Aligned | Supports result summary and answer-level educational feedback.                                            |
