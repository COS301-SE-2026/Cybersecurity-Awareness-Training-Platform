...
| --------------- | ----------------------------------- | ------------------- | ------------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------ |
| TRACE-TRK-01    | Simulated email opened/viewed       | UC-01               | TRK-UC01-01, TRK-UC01-02, TRK-UC01-03, FR-UC01-04 | API-TRK-01 / `POST /simulations/emails/:emailId/interactions` | `InteractionEvent`, `SimulatedEmail`, `LearnerProfile`                           | Records lightweight simulated inbox interaction events.                                | Draft  |
...
| TRACE-TRK-03    | Training document viewed/progressed | UC-02               | TRK-UC02-01, TRK-UC02-02, TRK-UC02-03, FR-UC02-04 | API-TRK-02 / `POST /training/:trainingId/progress`            | `TrainingDocument`, `CampaignItem`, `InteractionEvent`, `LearnerProfile`         | Tracks high-level training engagement through lightweight interaction events.          | Draft  |
...
| TRACE-TRK-05    | Quiz attempt creation               | UC-03               | TRK-UC03-01, FR-UC03-03                           | API-TRK-03 / `POST /quizzes/:quizId/attempts`                 | `QuizAttempt`, `Quiz`, `LearnerProfile`                                          | Starting a quiz creates an attempt.                                                    | Draft  |
...
| Training list/content    | Training document               | `GET /training/assigned`, `GET /training/:trainingId` | `CampaignAssignment`, `CampaignItem`, `TrainingDocument` | Aligned | Training documents are reusable content placed into campaigns through campaign items/components.          |
| Training progress        | Training interaction            | `POST /training/:trainingId/progress`                 | `InteractionEvent`                                       | Aligned | Records high-level training interaction only.                                                             |
