# Demo 2 API Contract Summary

## Purpose

This document is the Demo 2 API contract summary. It does not replace the Swagger API documentation.

The API is structured to support both trainee-facing campaign learning flows and platform administrative actions. In Demo 2, we introduce organization registration review endpoints for platform administrators.

UC-05 maps to the admin-scoped organisation registration review, contact, approval, and rejection endpoints documented below.
...
Swagger should be treated as the full interactive implementation-level API reference. This `API.md` file summarises the Demo 2 contract, use-case traceability, and high-level expectations at a stable documentation level.

## Contract Status Legend

| Status                     | Meaning                                                                               |
| -------------------------- | ------------------------------------------------------------------------------------- |
| Demo 2 required            | Needed for one of the core Demo 2 trainee or admin use cases                          |
| Demo 2 optional/supporting | Supports access, tracking, auditing, or usability, but is not a counted core use case |
| Future/later-demo          | Directional only; not required for Demo 2 implementation                              |
...
## UC-05: Admin Organisation Review Contracts

These endpoints allow Insightful Phish platform administrators (users with role `IP_ADMIN` and an active profile) to review and manage organisation registration requests.

### `GET /admin/organisation-registration-requests`

- **Purpose**: Lists all organisation registration requests
- **Status**: Demo 2 required
- **Access**: Requires Bearer auth and active `IP_ADMIN` role
- **Query Parameters**:
  - `status` (optional): Filter requests by state (`PENDING_REVIEW`, `CONTACTED`, `APPROVED`, `REJECTED`, `CANCELLED`)
  - `limit` (optional): Pagination limit
  - `offset` (optional): Pagination offset
- **Success response**: `200 OK` with an array of registration requests
- **Important fields**: `id`, `submittedOrganisationName`, `submittedWebsite`, `submittedOrganisationSize`, `representativeFirstName`, `representativeLastName`, `representativeEmail`, `status`, `createdAt`, `updatedAt`

### `GET /admin/organisation-registration-requests/:requestId`

- **Purpose**: Retrieves the detailed fields of a specific organisation registration request
- **Status**: Demo 2 required
- **Access**: Requires Bearer auth and active `IP_ADMIN` role
- **Success response**: `200 OK` with the complete request detail object
- **Notes**: Includes additional detailed fields like `submittedOrganisationDescription`, `representativePhone`, `rejectionReason`, and links to the resolving admin IDs.

### `POST /admin/organisation-registration-requests/:requestId/contact`

- **Purpose**: Marks a pending organisation registration request as manually contacted
- **Status**: Demo 2 required
- **Access**: Requires Bearer auth and active `IP_ADMIN` role
- **Success response**: `200 OK` with the updated request object (status is updated to `CONTACTED`)
- **Notes**: Audits the action in the `AuditLogEntry` table (`actionType: CONTACTED`, `targetType: ORGANISATION_REGISTRATION_REQUEST`, `outcome: SUCCESS`).

### `POST /admin/organisation-registration-requests/:requestId/approve`

- **Purpose**: Approves an organisation registration request
- **Status**: Demo 2 required
- **Access**: Requires Bearer auth and active `IP_ADMIN` role
- **Success response**: `201 Created` with the created `Organisation` details and the updated request status
- **Side effects**:
  - Creates the `Organisation` record.
  - Creates an admin-level `Invitation` for the representative.
  - Queues an email invite setup containing a secure `ActionToken`.
  - Audits the action in the `AuditLogEntry` table (`actionType: APPROVED`, `outcome: SUCCESS`).

### `POST /admin/organisation-registration-requests/:requestId/reject`

- **Purpose**: Rejects an organisation registration request
- **Status**: Demo 2 required
- **Access**: Requires Bearer auth and active `IP_ADMIN` role
- **Request Body**: `{ rejectionReason: string }`
- **Success response**: `200 OK` with the updated request object (status is updated to `REJECTED`)
- **Side effects**:
  - Queues a rejection email containing the reason.
  - Audits the action in the `AuditLogEntry` table (`actionType: REJECTED`, `outcome: SUCCESS`).
- **Notes**: A valid rejection reason must be provided (validated by shared schemas).
...
| 0.2.0   | 2026-07-01 | Rudolph Lamprecht                | Admin Organisation Review Contracts   | Updated for Demo 2; documented admin organisation review endpoints and RBAC rules.   |
