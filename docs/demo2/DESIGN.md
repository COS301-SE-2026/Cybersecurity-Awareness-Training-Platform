# Demo 2 Design Documentation

## Purpose

This document records the Demo 2 design baseline for Insightful Phish. It focuses on the trainee-facing and admin-facing brand styles, screen layout directions, navigation points, component placement, interaction points, and wireframe references needed to support the Demo 2 flow.

Demo 2 design covers the base authentication screens, the trainee use cases, and the admin management use case:
...
UC-05 uses the platform admin management surface and a request details flow to support list review, status changes, onboarding handoff, and audit-friendly rejection handling.
...
- Provide platform admins with a single list to track, search, filter, and access submitted organisation registration requests.

**Navigation points:**

- Accessed via the administration navigation header.
- Leads to the Organisation Registration Details Page when a request is clicked or details are selected.

**Component placement:**

- Page title: "Organisation Registration Requests"
- Filter and search controls: Status filters (All, Pending Review, Contacted, Approved, Rejected) and a search input for organisation name or representative email.
- Data table/list: Columns for Organisation Name, Representative, Representative Email, Submitted Date, Status.
- Status badges: Colored status indicators matching the request state.
- Empty state: Friendly layout when no requests match the active filter.
- Pagination controls.

**User interaction points:**

- Click status filters to narrow the list.
- Type in search input to filter by name/email.
- Click a request row or "View Details" action to navigate to the details page.
- Review and recover from list retrieval errors via a retry action.

**Annotations:**

- The list should display requests sorted by submission date descending by default.
- Access is strictly guarded; non-admin users must be redirected or blocked before loading the component.
- The layout must fit desktop and tablet dimensions comfortably, using responsive overflow if viewed on smaller screens.

### Organisation Registration Details Page

**Purpose:**

- Present the platform admin with the complete set of submitted details for a single request, and provide options to review, contact, approve, or reject it.

**Navigation points:**

- Entered from the Organisation Management Page.
- Returns to the Organisation Management Page via a back link or button.

**Component placement:**

- Back navigation link: "< Back to Requests"
- Request header: Organisation Name, Date Submitted, Current Status badge.
- Details panel:
  - Organisation Info: Name, Website URL, Description, Approximate Size.
  - Representative Info: Full Name (First & Last), Email, Phone Number.
- Decision panel:
  - "Mark Contacted" button (enabled only when status is `PENDING_REVIEW`).
  - "Approve" button (enabled only when status is `PENDING_REVIEW` or `CONTACTED`).
  - "Reject" button (enabled only when status is `PENDING_REVIEW` or `CONTACTED`).
- Rejection dialog/form: Text area for "Rejection Reason" (visible/activated when the "Reject" action is selected).
- History/Audit context: If already processed, shows when and which admin performed the action.

**User interaction points:**

- Click the website URL to open it in a new window/tab.
- Select "Mark Contacted" to change status.
- Select "Approve" (prompts confirmation dialog, transitions status, and triggers setup email).
- Select "Reject" (prompts dialog, requires rejection reason text, and transitions status).
- Click back link to return to the list.

**Annotations:**

- Rejection reason input must have client-side character limit validation (maximum 2000 characters) and required field checks.
- When an action is in progress (approving/rejecting), buttons must show a loading spinner and be disabled to prevent duplicate requests.
- Upon successful action, show a page-level success banner and transition the page to read-only.
...
| 0.2.0   | 2026-07-01 | Rudolph Lamprecht | Title, Intro, Admin review screens, History | Updated for Demo 2; documented layout and UX expectations for UC-05. |
