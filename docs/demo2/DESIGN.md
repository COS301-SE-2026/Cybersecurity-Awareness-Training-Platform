...
- UC-07 should provide an invitation acceptance page that shows safe token context, the target email, the organisation name, the accepted role or setup context, and clear success or refusal states for expired, revoked, already-used, unsupported, or wrong-user invitations.
- UC-07 screens should keep the raw token hidden, preserve organisation scope, and make it obvious when the user must switch accounts or cannot complete the invitation because the target identity does not match.
...
- UC-07, UC-09, and UC-11 should preserve the existing feedback, validation, accessibility, loading, success, warning, and error-state rules in this design document.
...
| 0.1.8   | 2026-07-16 | Rudolph Lamprecht | UC-07 invitation acceptance design references | Added later-demo invitation acceptance design references.             |
