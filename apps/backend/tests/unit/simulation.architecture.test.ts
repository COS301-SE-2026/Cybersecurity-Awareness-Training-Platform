...
describe('Simulation Architecture Boundaries', () => {
  const servicePath = resolveBackendFilePath('services/simulation.service.ts');
  const controllerPath = resolveBackendFilePath('controllers/simulation.controller.ts');
  const routesPath = resolveBackendFilePath('routes/trainee.routes.ts');

  it('prohibits direct Prisma imports in simulation.service.ts', () => {
    ...
    for (const statement of importStatements) {
      ...
      expect(statement).not.toMatch(/\/generated\/prisma/i);
    }
    ...
    for (const repoImport of repositoryImports) {
      expect(repoImport).toMatch(/simulation\.repository/);
    }
  });

  it('ensures simulation controller does not import Prisma or repositories directly', () => {
    const controllerContent = fs.readFileSync(controllerPath, 'utf-8');
    expect(controllerContent).not.toMatch(/from\s+['"].*prisma.*['"]/i);
    expect(controllerContent).not.toMatch(/from\s+['"].*repository.*['"]/i);
  });

  it('ensures trainee routes do not import Prisma or repositories directly', () => {
