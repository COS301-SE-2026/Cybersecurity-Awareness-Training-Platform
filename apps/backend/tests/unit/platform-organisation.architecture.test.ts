...
describe('Platform Organisation Architecture Boundaries', () => {
  const servicePath = resolveBackendFilePath('services/platformOrganisation.service.ts');
  const controllerPath = resolveBackendFilePath('controllers/platformOrganisation.controller.ts');
  const routesPath = resolveBackendFilePath('routes/platform.routes.ts');
  const repoPath = resolveBackendFilePath('repositories/organisation.repository.ts');

  it('prohibits direct Prisma imports and database infrastructure in platformOrganisation.service.ts', () => {
    ...
    for (const repoImport of repositoryImports) {
      expect(repoImport).toMatch(/organisation\.repository/);
    }
  });

  it('ensures platformOrganisation controller does not import Prisma or repositories directly', () => {
    const controllerContent = fs.readFileSync(controllerPath, 'utf-8');
    expect(controllerContent).not.toMatch(/from\s+['"].*prisma.*['"]/i);
    expect(controllerContent).not.toMatch(/from\s+['"].*repository.*['"]/i);
  });

  it('ensures platform routes do not import Prisma or repositories directly', () => {
    const routesContent = fs.readFileSync(routesPath, 'utf-8');
    expect(routesContent).not.toMatch(/from\s+['"].*prisma.*['"]/i);
    expect(routesContent).not.toMatch(/from\s+['"].*repository.*['"]/i);
  });

  it('ensures organisation repository does not import application services', () => {
