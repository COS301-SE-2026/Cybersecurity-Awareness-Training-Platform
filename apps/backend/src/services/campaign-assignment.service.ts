...
<<<<<<< HEAD
  const assignableCampaigns = await findAssignableCampaignsByIds({
    organisationId,
    campaignIds: uniqueCampaignIds,
  });

  if (assignableCampaigns.length !== uniqueCampaignIds.length) {
    throw new CampaignAssignmentServiceError(
      404,
      'CAMPAIGN_NOT_FOUND',
      'One or more specified campaigns were not found, inactive, or not eligible for assignment',
    );
  }

  const eligibleTrainees = await findEligibleTraineesByIds({
    organisationId,
    traineeProfileIds: uniqueTraineeProfileIds,
  });

  if (eligibleTrainees.length !== uniqueTraineeProfileIds.length) {
    throw new CampaignAssignmentServiceError(
      404,
      'TRAINEE_NOT_FOUND',
      'One or more specified trainees were not found, inactive, or not eligible for assignment',
    );
  }
