...
<<<<<<< HEAD
    const existingKeySet = new Set(
      existing.map((row) => `${row.campaignId}:${row.traineeProfileId}`),
    );

    const toCreate: Prisma.CampaignAssignmentCreateManyInput[] = [];
...
=======
    ...
    if (missingPairs.length > 0) {
      ...
      countInsertedCount = createRes.count;
    }

    // 5. Query all assignments after insertion
>>>>>>> a14d5b721 (feat: manage organisation campaign assignments transactionally (#407))
    const allAssignments = await tx.campaignAssignment.findMany({
      where: {
        campaignId: { in: input.campaignIds },
        traineeProfileId: { in: input.traineeProfileIds },
      },
